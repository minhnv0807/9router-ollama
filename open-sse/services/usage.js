...
const ANTIGRAVITY_CONFIG = {
  ...
  clientId: process.env.ANTIGRAVITY_CLIENT_ID || "YOUR-GOOGLE-CLIENT-ID.apps.googleusercontent.com",
  clientSecret: process.env.ANTIGRAVITY_CLIENT_SECRET || "YOUR-GOOGLE-CLIENT-SECRET",
...
export async function getUsageForProvider(connection) {
  ...
  switch (provider) {
    ...
    case "ollama":
      return await getOllamaUsage(accessToken || apiKey, providerSpecificData);
    default:
 ...
 * There is no public usage/quota API, so we return connection status.
 * The actual usage is tracked per-request via usageTracking.js.
 */
async function getOllamaUsage(accessToken, providerSpecificData) {
  try {
    // accessToken is the apiKey for APIKEY_PROVIDERS
    const apiKey = accessToken;
    const plan = providerSpecificData?.plan || "Pro";

    // If apiKey is present, show as "Connected" so key rotation UI works
    if (apiKey) {
      return {
        plan,
        message: "Ollama Cloud connected. Usage tracked per request. Key rotation enabled for Pro accounts.",
        quotas: {
          "api-key": {
            used: 0,
            total: 1,
    ...
    return {
      plan,
      message: "No API key configured. Add your Ollama Cloud API key at ollama.com/settings/keys.",
      quotas: {},
    };
  } catch (error) {
